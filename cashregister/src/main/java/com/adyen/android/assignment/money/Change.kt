...
class Change {
    ...
    //get the currency type saved in the map
    fun getElements(): Set<MonetaryElement> {
        return map.keys
    }

    //calculate the number of each currency type in the map
    fun getCount(element: MonetaryElement): Int {
        return map[element] ?: 0
    }

    //add currency type and the number of each currency type in the map
    fun add(element: MonetaryElement, count: Int): Change {
        return modify(element, count)
    }

    //removes each currency type from the map plus how many of each currency should be removed from
    // the map.
